...
import { FaArrowRight, FaCalendarDays, FaLightbulb } from "react-icons/fa6";
                ...
                <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
                    <div className="mb-12 grid gap-6 rounded-[2rem] border border-slate-200 bg-slate-50 p-7 md:grid-cols-[1.2fr_0.8fr] md:items-center md:p-10">
                        <div>
                            <div className="flex items-center gap-3">
                                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#100484] text-[#03DAC6]">
                                    <FaLightbulb />
                                </div>
                        ...
                        <div className="rounded-3xl bg-white p-6 shadow-sm">
                            <p className="text-4xl font-black text-[#ff7d09]">12+</p>
                    ...
                    <div className="grid gap-7 md:grid-cols-2 lg:grid-cols-3">
                        {blogs.map((post, index) => (
                            <article
                                key={post.title}
                                className="group relative flex min-h-[360px] flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-slate-50 p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#ff7d09]/30 hover:bg-white hover:shadow-2xl hover:shadow-slate-200/70"
                            >
                                <div className="absolute -right-6 -top-6 text-7xl font-black text-slate-100 transition group-hover:text-[#ff7d09]/15">
                                ...
                                >
                                    ...
                                    <div className="absolute left-4 top-4">
                                        <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-black uppercase tracking-wider text-[#ff7d09] backdrop-blur">
                                            {post.category}
                                        </span>
                                    </div>
                                </Link>
                                ...
                                <h2 className="relative mt-6 text-2xl font-black leading-tight tracking-tight">
                                    <Link
                                        to={`/insights/${post.slug}`}
                                        className="text-slate-950 transition-colors duration-300 hover:text-[#ff7d09]"
                                    >
                                        {post.title}
                                    </Link>
                                </h2>
                                ...
                                <div className="relative mt-6 h-px w-full bg-gradient-to-r from-[#ff7d09]/35 via-[#1667FF]/20 to-transparent" />

                                <div className="relative mt-auto flex items-center justify-between pt-6">
                                    <div className="flex items-center gap-2 text-sm font-semibold text-slate-500">
                                        <FaCalendarDays className="text-[#ff7d09]" />
                                        {post.date}
                                    </div>
